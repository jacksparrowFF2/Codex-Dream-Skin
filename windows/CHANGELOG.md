...
- 原生用户消息导航轨道现在采用 EVA 插入栓深度计样式：已完成、当前和后续刻度具有独立层级，当前阶段显示 `Pxx · SYNC/RUN/HOLD`；带图片的轨道预览增加 `VISUAL RECORD`/`RENDER COMPLETE` 记录标题，仍保留原生按钮、滚动和预览行为。
- 新增 Rainmeter/NERV 风格的功能仪表层：环境摘要中显示 MAGI 三核心状态、仅从官方 Usage 设置表面读取的剩余用量，以及明确标为 `EST.` 的上下文占用估算；当前任务、输入区和文件变更卡同步提供离散状态与操作编号，所有装饰层均不拦截原生交互。
...
- 浅色模式改为冷蓝白局部玻璃层：任务背景右侧显著降低全局白雾以恢复人物和夜景层次，正文侧仍保留渐变阅读保护；输入框、文件卡、菜单和 MAGI 摘要使用更实的浅色表面与蓝灰边框，不再把整张壁纸和所有功能区域洗成相近的灰白色。
- 当窗口明显宽于当前壁纸比例时，整窗背景会由 `cover` 自动切换为按视口高度完整显示，避免缩小窗口高度后右侧人物的头顶、裙摆或鞋被上下裁切；恢复常规比例后自动回到沉浸式铺满模式。
- 文件变更卡改用 `OPERATION REPORT · FILES xx` 标题；轨道预览、阶段游标及操作报告统一采用 11–12px 的最低可读字号，窄窗口会隐藏阶段文字以避免覆盖正文。
- MAGI 用量仪表不再依赖手动打开“状态”：只读复用 Codex 每分钟更新的 `rate-limit-status` 查询缓存，并从当前会话的 `latestTokenUsageInfo` 获取上下文占用；不会发送状态命令或增加网络请求，自动读数以 `AUTO` 标识。
- MAGI 面板的标题、三核心名称与状态、额度标签、百分比和读数元数据统一提升字号层级，在环境信息栏宽度下保持清晰且不截断。
- MAGI 仪表现在优先读取会话内原生“状态”卡片：7 天限额与重置时间驱动账户用量条，背景信息驱动当前会话上下文剩余量；关闭卡片后保留最近一次官方读数，并按会话 UUID 隔离上下文缓存，避免切换任务时串值。
- 当前任务悬停或键盘聚焦时，`SYNC` 徽标会为原生“置顶任务/归档任务”按钮自动左移并增加标题留白；侧栏拖拽分界手柄移到滚动区域外侧，滚动条改为独立窄轨，不再叠成粗灰带。
- 会话区与右侧工作区之间同样拆分滚动条和拖拽手柄：会话滚动条保留在内容内侧，8px 分栏手柄完整移入右侧工作区，避免右边界出现重叠的宽竖条。
- 收起左侧边栏时不再把完整 Codex 主窗口误判为辅助窗口并清除皮肤；任务同步徽标为原生尾部图标预留独立空间，MAGI 三核心及用量/上下文仪表改用更大的字号与纵向全宽布局。
- 浅色 Codex 外观保存的紫色原生 token 现在会随当前角色主题重新映射：绫波丽主题的链接、焦点、按钮与交互强调恢复为蓝色；环境信息、来源、附件“添加/插件”标题及侧栏底部等原生表面同步使用主题前景、边框与半透明表面，不再残留粉紫色。
- “添加文件等内容”面板没有菜单角色，现按其稳定的 320px 弹层结构标记并应用主题玻璃表面；首页项目工具栏不再写死 `-20px` 补偿，而是比较原生工具栏与 composer 的左右边界后动态计算 margin 与 padding，兼容空白新会话和已有草稿两种布局。
- 任务右侧工具抽屉与“环境信息”摘要卡并非语义菜单，现由渲染器按稳定结构分别标记；抽屉的多层 `bg-token-main-surface-primary` 实色背景会被合并为单一主题玻璃层，摘要卡也使用独立半透明表面，不再遗漏截图中的两类纯色区域。
- 原生 `menu`/`listbox` 二级弹层现在使用主题表面色、18px 背景模糊、主题边框与强调色选中态，不再显示为与壁纸割裂的固定纯色块；普通内容卡片和大型对话框保持原有可读性。
- 自定义十六进制强调色现在按主题实际颜色计算按钮前景色，不再错误沿用壁纸分析色；蓝色、紫色等角色主题可自动选择对比度更高的深色或浅色图标。
- 首页输入区的项目工具栏现在会补偿 Codex 原生的 20px 内容缩进，与 composer 外沿对齐；同时为项目、本地、分支、模型、附件和占位文字提供明确的前景色与悬停态，并增强深色侧栏、浅色首页、顶栏、建议卡和输入面板的阅读层，避免 EVA 等高细节壁纸降低界面元素对比度。EVA 预设改为跟随 Codex/系统外观，不再把切换后的浅色原生外壳与固定深色皮肤混用。
