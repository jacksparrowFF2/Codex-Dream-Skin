...
try {
  ...
  foreach ($releaseAsset in @(
    'dream-skin.css', 'eva-compat.css', 'eva-compat.js', 'renderer-inject.js',
    'safe-css-policy.json', 'safe-css-validator.mjs', 'selectors.json',
  ...
  $evaCompatTest = Invoke-DreamSkinNative -FilePath $node.Path -ArgumentList @(
    (Join-Path $PSScriptRoot 'eva-compat.test.mjs'))
  if ($evaCompatTest.ExitCode -ne 0) { throw 'EVA compatibility layer regression test failed.' }
