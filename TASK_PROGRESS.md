...
## EVA compatibility layer on upstream v1.5.14 — locally complete (2026-08-19)

- [objective] Preserve all twelve EVA themes and their Windows MAGI/office
  treatment on the current upstream engine while retaining upstream startup,
  rollback, localization, import, and security behavior.
- [scope] Independent worktree
  `output/worktrees/Codex-Dream-Skin-eva-v1.5.14` on branch
  `codex/eva-compat-v1.5.14`. The original derived `main` worktree and the
  installed `%LOCALAPPDATA%` engine remain untouched.
- [merged locally] `upstream/main` v1.5.14 (`95423d8`) is merged into the
  derived base. The two content conflicts were resolved by retaining the
  canonical upstream renderer/test entry and moving EVA behavior into a
  Windows-only extension.
- [implemented locally] `windows/assets/eva-compat.js` adds theme-ID-gated
  MAGI, composer status, operation report, thread rail, and office treatment;
  `eva-compat.css` supplies isolated styling. The injector composes both files
  into the normal payload and installers now require/package them.
- [compatibility] The shared selector contract recognizes Codex 26.810/26.814
  composer data attributes and upper-case Markdown module names. A new Node
  test exercises activation, isolation, cleanup, and all twelve EVA IDs.
- [verified locally] Runtime asset sync, JavaScript syntax, both platform
  renderer/payload integrity checks, installer static checks, selector doctor,
  canonical renderer tests, theme schema tests, localization tests, and the EVA
  compatibility test pass. The complete Windows regression suite passes after
  the final payload-boundary fix.
- [theme validation] A disposable store installed all twelve EVA themes; every
  theme compiled through the v1.5.14 Windows payload with `pass: true` and
...
# Codex latest-layout compatibility refresh (2026-08-14)

- Goal: diagnose why the EVA theme no longer covers or exposes the complete layout after the current Windows Codex update, then update the source configuration/runtime compatibility layer and validate it.
- Scope: read-only inspection of the installed Codex/Dream Skin state; derived-engine selector, renderer, and shared CSS sources only where current evidence requires changes; parent EVA theme metadata only if image composition settings are the cause.
- Worktrees: parent `main`; derived engine `main`; both clean at task start. Existing untracked parent `output/` diagnostics are user-owned and will be preserved.
- Completed diagnosis: installed Codex is `26.810.4967.0` / Chromium `151.0.7922.137`. The wallpaper, shell, sidebar, and document overflow checks remained healthy, but the old `.composer-surface-chrome`, `_footer_`, and lowercase `_markdown` probes all missed the current renderer.
- Completed implementation: retained every legacy selector while adding the current `data-composer-surface-variant` + `data-composer-layout` shell, `data-composer-footer-responsive` toolbar, and uppercase `_Markdown*` module prefixes to the shared contract and generated cross-platform assets. The derived Windows EVA renderer and CSS now use the same backward-compatible composer selector without losing MAGI/EVA behavior.
- Verified against live DOM: selector Doctor reports L1 PASS and current L2 counts `composer-chrome=1`, `composer-toolbar=1`, `markdown=91`, `thread-surface=1`, `message=4`.
- Verified regressions: focused selector/renderer/payload tests pass; the complete Windows `run-tests.ps1` passes outside the sandbox (the sandbox-only run hit the expected controlled-child-process `spawn EPERM`); all EVA JSON parses, disposable installation installs 12 complete themes, Safe CSS validates with 11 rules / 26 declarations, and both worktrees pass `git diff --check`.
- Remaining deployment: source is fixed but the installer-managed `%LOCALAPPDATA%\CodexDreamSkin\engine` was not edited or reinstalled while this Codex task is open. Close Codex and the Dream Skin tray before intentionally running the normal installer to deploy the updated engine.
- Follow-up finding: Codex 26.810 also changed the right environment-summary surface from `bg-token-dropdown-background` to `bg-surface-elevated-secondary` and added `data-slot="thread-summary-panel-item-button"`. The old detector therefore removed the MAGI/usage module. The derived Windows detector now accepts both generations and still requires a summary-item anchor before injecting.
- Follow-up verification: the captured 26.810 renderer contains exactly one compatible summary candidate and five semantic summary items. Focused renderer/payload/syntax checks and the complete Windows regression suite pass after the MAGI detector update.
